...
class BasicGraphModel(nn.Module):  # Baseline model
...
class LinearModel(nn.Module):  # Linear model - for experiment purpose
    def __init__(self, g, n_layers, input_size, hidden_size, output_size, nonlinearity):
        super().__init__()
...
class AttentionGraphModel(nn.Module):  # GAT model - best model
    def __init__(
        self,
        g,
        n_layers,
        ...
        self.layers.append(
            GATConv(
                input_size, hidden_size, num_heads=num_heads, activation=nonlinearity
            )
        )
        for i in range(n_layers - 1):
            self.layers.append(
    ...
    def forward(self, inputs):
        ...
        for i, layer in enumerate(self.layers):
            outputs = layer(self.g, outputs)
            outputs = outputs.flatten(1)  # to get (batch_size, num_heads * hidden_size)
        logits_multiple_heads = self.output_layer(self.g, outputs)
        output = torch.mean(
            logits_multiple_heads, dim=1
        )  # average on the num_heads dimension to get final predictions
        return output
...
def main(args):
    ...
    train_dataset, test_dataset = LegacyPPIDataset(mode="train"), LegacyPPIDataset(
        mode="test"
    )
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 5 == 0:
            ...
            for batch, test_data in enumerate(test_dataset):
                ...
                score, _ = evaluate(
                    features.float(), model, subgraph, labels.float(), loss_fcn
                )
                scores.append(score)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gpu", type=int, default=-1, help="GPU to use. Set -1 to use CPU."
    )
    parser.add_argument("--epochs", type=int, default=100)  # default=250
